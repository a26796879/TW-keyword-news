...
class NewsCrawler:
    ...
    async def get_udn_news(self, async_session, keyword):
        ...
        udn_url = f'https://udn.com/api/more?page=0&id=search:\
    ...
    async def get_apple_news(self, async_session, keyword):
        ...
        apply_url = f'https://tw.nextapple.com/search/{urllib.parse.quote_plus(keyword)}/1'
        res = await async_session.get(url=apply_url, headers=self.headers)
        all_page_soup = BeautifulSoup(res.text, 'html.parser')
        main_content = all_page_soup.find_all("div", id='main-content')
        ...
        for i, each_news in enumerate(publish):
            ...
            # 臺蘋沒有按照時間排序 必須for完整個list
        return results

    async def get_setn_news(self, async_session, keyword):
        ''' get setn news by crawling'''
        url = f'https://www.setn.com/search.aspx?q={urllib.parse.quote_plus(keyword)}&r=0'
    ...
    async def get_ettoday_news(self, async_session, keyword):
        ...
        url = f'https://www.ettoday.net/news_search/doSearch.php?search_term_string=\
    ...
    async def get_tvbs_news(self, async_session, keyword):
        ...
        url = f'https://news.tvbs.com.tw/news/searchresult/{urllib.parse.quote_plus(keyword)}/news'
        res = await async_session.get(url=url, headers=self.headers)
        soup = BeautifulSoup(res.text, 'html.parser')
    ...
    async def get_china_news(self, async_session, keyword):
        ...
        url = f'https://www.chinatimes.com/search/{urllib.parse.quote_plus(keyword)}?chdtv'
        res = await async_session.get(url=url, headers=self.headers)
        soup = BeautifulSoup(res.text, 'html.parser')
    ...
    async def get_storm_news(self, async_session, keyword):
        ...
        url = f'https://www.storm.mg/site-search/result?q={urllib.parse.quote_plus(keyword)}\
            &order=none&format=week'
        res = await async_session.get(url=url, headers=self.headers)
        soup = BeautifulSoup(res.text, 'html.parser')
    ...
    async def get_ttv_news(self, async_session, keyword):
        ...
        url = f'https://news.ttv.com.tw/search/{urllib.parse.quote_plus(keyword)}'
        res = await async_session.get(url=url, headers=self.headers)
        soup = BeautifulSoup(res.text, 'html.parser')
    ...
    async def get_ftv_news(self, async_session, keyword):
        ...
        url = f'https://www.ftvnews.com.tw/search/{urllib.parse.quote_plus(keyword)}'
        res = await async_session.get(url=url, headers=self.headers)
        soup = BeautifulSoup(res.text, 'html.parser')
    ...
    async def get_cna_news(self, async_session, keyword):
        ...
        url = f'https://www.cna.com.tw/search/hysearchws.aspx?q={urllib.parse.quote_plus(keyword)}'
        res = await async_session.get(url=url, headers=self.headers)
        soup = BeautifulSoup(res.text, 'html.parser')
        ...
        for value in enumerate(urls):
            ...
            if published_date >= expect_time:
                results.append({
                    'title': titles[value[0]].text,
                    'url': f'https://www.cna.com.tw/{urls[value[0]].get("href")}',
                    'publisher': 'CNA中央社',
                    'published_date': published_date
                })
            else:
    ...
    async def get_ltn_news(self, async_session, keyword):
        ...
        for value in tit_tag:
            ...
            res = await async_session.get(url=each_url, headers=self.headers)
            soup = BeautifulSoup(res.text, 'html.parser')
            if 'health.ltn' in each_url or 'sports.ltn' in each_url or \
                'ec.ltn' in each_url or 'def.ltn' in each_url:
